...
async function assertRevertFailure(world: World, err: string, message: string): Promise<World> {
  ...
  const expectedMessage = `Returned error: VM Exception while processing transaction: ${coverageSafeRevertMessage(world, message)}`;
...
function buildRevertMessage(world: World, message: string): string {
  return `Returned error: VM Exception while processing transaction: ${coverageSafeRevertMessage(world, message)}`
}
