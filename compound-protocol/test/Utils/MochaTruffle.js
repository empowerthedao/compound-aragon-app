...
const assert = Object.assign(global.assert || require('assert'), {
  ...
  hasOracleError: (actual, expectedErrorName) => assert.hasError(actual, expectedErrorName, OracleErr),
  ...
  hasOracleFailure: (result, err, info, detail=undefined) => assert.hasFailure(result, err, info, detail, OracleErr),
  ...
  revert: async (trx, reason='revert') => {
    ...
    let safeReason = process.env.NETWORK === 'coverage' ? 'revert' : reason;
    let result;
    try {
      result = await trx;
    } catch (err) {
      assert.equal(err.message, `Returned error: VM Exception while processing transaction: ${safeReason}`);
      return;
    }
  ...
  oracleSuccess: (result) => assert.success(result, OracleErr),
